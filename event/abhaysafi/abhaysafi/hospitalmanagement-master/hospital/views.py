...
def download_pdf_view(request,pk):
    ...
    return render_to_pdf('college/download_bill.html',dict)
...
def head_login(request):
    if request.method == 'POST':
        ...
        if username == 'principal' and password == '9876':
            ...
            # Log in the user
            login(request, user)
            return redirect('prin-appointment')
        else:
...
def p_admin_appointment_view(request):
    return render(request,'prin/admin_appointment.html')
...
def p_view_staff_view(request):
    staffs=models.staff.objects.all().filter(status=True)
    return render(request,'prin/student_view_staff.html',{'staffs':staffs})


def p_view_student_view(request):
    students=models.student.objects.all().filter(status=True)
    return render(request,'prin/staff_view_student.html',{'students':students})
